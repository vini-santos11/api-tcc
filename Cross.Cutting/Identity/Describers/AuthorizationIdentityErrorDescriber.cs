...
namespace Cross.Cutting.Identity.Describers
{
    public class AuthorizationIdentityErrorDescriber : IdentityErrorDescriber
    {
        ...
        public override IdentityError DuplicateEmail(string email) { return new IdentityError { Code = nameof(DuplicateEmail), Description = $"O e-mail '{email}' já está sendo usado por outro usuário, não é permitido outro cadastro com o mesmo e-mail. O campo e-mail é opcional, caso você não tenha um e-mail particular não é necessário informar." }; }
        public IdentityError DuplicatePhone(string phone) { return new IdentityError { Code = nameof(DuplicatePhone), Description = $"O número de telefone '{phone}' já está sendo usado por outro usuário, não é permitido outro cadastro com o mesmo número." }; }
        public IdentityError DuplicateDocument(string document) { return new IdentityError { Code = nameof(DuplicateDocument), Description = $"O CPF '{document}' já está sendo usado por outro usuário." }; }
